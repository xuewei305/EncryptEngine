	   ...
	   )
{
	...
	try{
		...
		RtlCopyMemory(psFileFlag, g_psFileFlag, FILE_FLAG_LENGTH) ;
		...
		)
{
	...
	//Getting minifilter lower device object
	...
	)
{
	...
	try{
		...
		while (TRUE)
		{
			...
			status = File_ReadWriteFile(IRP_MJ_READ,FltObjects->Instance,FileObject,&ReadWriteOffset,uAllocateBufferSize,Buffer,&uReadBytes,FLTFL_IO_OPERATION_NON_CACHED|FLTFL_IO_OPERATION_DO_NOT_UPDATE_BYTE_OFFSET) ;
			if (!NT_SUCCESS(status))
				break;
			if (0 == uReadBytes)
				break;
