...
Abstract:
    ...
    This is a sample filter which demonstrates proper access of data buffer and a general guideline of how to swap buffers.
    ...
    By default this filter attaches to all volumes it is notified about.  It does support having multiple instances on a given volume.
...
KSPIN_LOCK	g_SpinLockForProcessRecognition;
...
--*/
{
    ...
    //
    //  Init lookaside list used to allocate our context structure used to pass information from out preOperation callback to our postOperation
    //  callback.
    //

	KeInitializeSpinLock(&g_SpinLockForProcessRecognition);
...
*/
	...
	RtlZeroMemory(g_szCurFileKeyDigest, HASH_SIZE);
    //
...
--*/
{
	...
	if (g_psFileFlag!=NULL)
	{
		File_UninitFileFlag() ;
	}

	//remove process create callback routine.
	//PsSetCreateProcessNotifyRoutine(Ps_ProcessCallBack, TRUE) ;
  
	//DbgPrint("Leave FilterUnload!\n");
...
--*/
{
    ...
    try {
        ...
        status = FltAllocateContext( FltObjects->Filter, FLT_VOLUME_CONTEXT, sizeof(VOLUME_CONTEXT), PagedPool, &ctx );
        if (!NT_SUCCESS(status)) 
        {
        	if (ctx)
    		{
        ...
        if (!NT_SUCCESS(status)) 
		{
            ...
            ctx->Name.Buffer = ExAllocatePool(NonPagedPool,size);
    ...
    )
{
	...
	try{
		...
		RtlZeroMemory(szCurProcessName,64);
		Ps_GetProcessName(szCurProcessName, NULL) ;
		...
		if (!bNewCreatedOrNot)
		{
			...
			pStreamCtx->RefCount ++ ;
			//DbgPrint("******** Stream already exist szProcessName is %s,pStreamCtx->RefCount is %d\n",szCurProcessName,pStreamCtx->RefCount);
		...
		pStreamCtx->bHasExcelWriteData = FALSE;
		pStreamCtx->RefCount ++ ;
		
		//DbgPrint("******** Init new created stream context szProcessName is %s,pStreamCtx->RefCount is %d\n",szCurProcessName,pStreamCtx->RefCount);
...
{   
	...
	try{
		...
		// since this object is opened by other kernel component
			...
			{
				pStreamCtx->RefCount -- ;				
				RtlZeroMemory(szProcessName11,64);
				Ps_GetProcessName(szProcessName11, NULL) ;
				//DbgPrint("******** szProcessName11 is %s\n",szProcessName11);

				//DbgPrint("In PreClose B pStreamCtx->RefCount = %d \n",pStreamCtx->RefCount);
				//}
				...
				{//if reference decreases to 0, write file flag, flush|purge cache, and delete file context
					...
					//DbgPrint("******** pStreamCtx->FileName.Buffer:%S ,pStreamCtx->bHasWriteData = %d &  pStreamCtx->bIsFileCrypt = %d \n", pStreamCtx->FileName.Buffer,pStreamCtx->bHasWriteData, pStreamCtx->bIsFileCrypt);

					if (pStreamCtx->bHasWriteData)
					{
						//DbgPrint("******** pStreamCtx->bHasWriteData = TRUE");
					}

					if ((pStreamCtx->bHasWriteData || pStreamCtx->bIsFileCrypt) || (!pStreamCtx->bHasWriteData && !pStreamCtx->bIsFileCrypt && pStreamCtx->bHasPPTWriteData))
					{
					

						//get file dos full path
						RtlZeroMemory(wszFilePathName,520);
						RtlCopyMemory(wszFilePathName, pStreamCtx->FileName.Buffer, pStreamCtx->FileName.Length) ;//
						RtlZeroMemory(wszVolumePathName,128);
						RtlCopyMemory(wszVolumePathName, pStreamCtx->wszVolumeName, wcslen(pStreamCtx->wszVolumeName)*2) ;
						...
						RtlZeroMemory(wszFileDosFullPath,520);
						RtlCopyMemory(wszFileDosFullPath,pStreamCtx->FileName.Buffer, pStreamCtx->FileName.Length) ;
						...
						if (!NT_SUCCESS(status))
						{// open failed, pass
							//DbgPrint("_________________open failed, pass__________________\n");
							__leave ;
						}
						...
						if (!NT_SUCCESS(status))
						{
							//DbgPrint("get fileobject failed\n");
							__leave ;
						}

						if (pStreamCtx->bHasWriteData || pStreamCtx->bIsFileCrypt)
						{//write flag into end of file
							//DbgPrint("---------------------¡¾ File_WriteFileFlag ¡¿---------------------\n");
							File_WriteFileFlag(Data,FltObjects, FileObject, pStreamCtx) ;//Ã»ÓÐÃüÖÐ~~~
						}
						if (!pStreamCtx->bHasWriteData && !pStreamCtx->bIsFileCrypt && pStreamCtx->bHasPPTWriteData)
						{//only ppt related, encrypted entire file and add file flag in the end of file
							...
							//DbgPrint("---------------------¡¾here 3 File_UpdateEntireFileByFileObject ¡¿---------------------\n");
						...
						if((0 == pStreamCtx->RefCount)&&(bIsSystemProcess != EXCEL_PROCESS))
						{
		...
		if((bDeleteStreamCtx)&&(bIsSystemProcess != EXCEL_PROCESS))//if (bDeleteStreamCtx)//&& (bIsSystemProcess != EXCEL_PROCESS))
...
--*/
{
    ...
    try {
        ...
        //newBuf = ExAllocatePoolWithTag( PagedPool, readLen,BUFFER_SWAP_TAG );
		newBuf =ExAllocatePool(NonPagedPool,readLen);//
        ...
        try {
			do{
				...
				if (p2pCtx->pStreamCtx->bIsFileCrypt || p2pCtx->pStreamCtx->bDecryptOnRead)
				{
					...
					KdPrint(("======================== data_decrypt ====================================="));
					data_decrypt( (unsigned char *)(p2pCtx->SwappedBuffer),(unsigned int)( Data->IoStatus.Information) );
			...
			do{
				...
				if (p2pCtx->pStreamCtx->bIsFileCrypt || p2pCtx->pStreamCtx->bDecryptOnRead)
				{
					...
					///{// use file specified key to decrypt file data(this must be history key)
						...
						///}
					///}
					data_decrypt(p2pCtx->SwappedBuffer,Data->IoStatus.Information );
					KeLeaveCriticalRegion() ;
				}
...
--*/
{
    ...
    try {
		...
		// and encrypted in pre-close routine, ppt file is encrypted when it is closed.
		if (!pStreamCtx->bIsFileCrypt && !FltObjects->FileObject->WriteAccess)//
		{
			...
			{// for exception of powerpnt process, we just set write flag for the file
				//DbgPrint("A pStreamCtx->bHasPPTWriteData = TRUE£¡\n"); 
		...
		// If user click "save" button in powerpnt, no tmp file generated and partial data of the file is flush back to disk.
		...
		if (bIsPPTFile && !pStreamCtx->bIsFileCrypt && (bIsSystemProcess == POWERPNT_PROCESS))
		{
			...
			//DbgPrint("B pStreamCtx->bHasPPTWriteData = TRUE£¡\n"); 
			pStreamCtx->bHasPPTWriteData = TRUE ;
			SC_UNLOCK(pStreamCtx, OldIrql) ;		
			__leave ;
        ...
        //newBuf = ExAllocatePoolWithTag( PagedPool, ,BUFFER_SWAP_TAG );
		newBuf =ExAllocatePool(NonPagedPool,writeLen);//
        if (newBuf == NULL) 
            leave;
        ...
        try {
			if((newBuf!=NULL)&&(origBuf!=NULL)&&(writeLen!=0))
			...
			do {
				...
				{
					KeEnterCriticalRegion() ;
					KdPrint(("======================== data_encrypt ====================================="));
					///if (NULL == pStreamCtx->aes_ctr_ctx)
					{// use current key to decrypt file data

						if((newBuf!=NULL)&&(writeLen!=0))
							if (data_encrypt(newBuf, writeLen))//if (data_decrypt(newBuf, writeLen))
							{
								KeLeaveCriticalRegion() ;
								break ;
							}
    ...
    )
{
	...
	{
		//DbgPrint("************* In PostWrite p2pCtx->pStreamCtx->bHasWriteData = TRUE \n");
	...
	{
		if (bIsSystemProcess != EXCEL_PROCESS)
			Cc_ClearFileCache(FltObjects->FileObject, TRUE,&Data->Iopb->Parameters.Write.ByteOffset, Data->Iopb->Parameters.Write.Length);
	}
...
--*/
{
    ...
    try {
        ...
        //newBuf = ExAllocatePoolWithTag( PagedPool,iopb->Parameters.DirectoryControl.QueryDirectory.Length,BUFFER_SWAP_TAG );
        newBuf =ExAllocatePool(NonPagedPool,iopb->Parameters.DirectoryControl.QueryDirectory.Length);
		if (newBuf == NULL) 
            leave;
...
This routine enables   to find a unicode string mode in another unicode string, the arguments provider should make sure that the parameters are validate.

PUNICODE_STRING target - the string which would be iterated

PUNICODE_STRING mode - the string we are interested in.

ULONG startPos - the beginning search pos.

return

         -1 if not found, or return beginning pos zero-indexed based.

--*/
LONG 
FindUnicodeString(__in PUNICODE_STRING target, __in PUNICODE_STRING mode, __in ULONG startPos)
{
	LONG index = 0, pos = 0;
	PWCHAR tar = NULL;
	PWCHAR mod = NULL;
	...
	LOG_PRINT( LOGFL_FIND_UNICODESTRING, ("target->Length =%d\n",target->Length ));
	LOG_PRINT( LOGFL_FIND_UNICODESTRING, ("startPos =%d\n",startPos ));
	LOG_PRINT( LOGFL_FIND_UNICODESTRING, ("mode->Length =%d\n",mode->Length ));
	  
	if ( target->Length < startPos + mode->Length)
	{

	   LOG_PRINT( LOGFL_FIND_UNICODESTRING, ("target->Length < startPos + mode->Length return -1\n" ));

	   return -1;
	}

	tar = target->Buffer + (startPos>>1);
	mod = mode->Buffer;

	LOG_PRINT( LOGFL_FIND_UNICODESTRING, ("tar =%C\n",*tar ));
	LOG_PRINT( LOGFL_FIND_UNICODESTRING, ("mod =%C\n",*mod ));

	while (*tar++ != *mod)
	{

	   LOG_PRINT( LOGFL_FIND_UNICODESTRING, ("tar =%C\n",*tar ));
	   LOG_PRINT( LOGFL_FIND_UNICODESTRING, ("startPos++\n" ));
	   pos++;
	}
	pos <<= 1;

	tar--;
	LOG_PRINT( LOGFL_FIND_UNICODESTRING, ("tar--\n" ));
	...
	while (*tar++ == *mod++)
	{
	   ...
	   {
	    index++;
	    LOG_PRINT( LOGFL_FIND_UNICODESTRING, ("index++\n" ));
	   }

	   if ((index+1) * 2> mode->Length)
	   {
	...
	LOG_PRINT( LOGFL_FIND_UNICODESTRING, ("index =%d\n",index ));
	LOG_PRINT( LOGFL_FIND_UNICODESTRING, ("pos =%d\n",pos ));
	LOG_PRINT( LOGFL_FIND_UNICODESTRING, ("startPos =%d\n",startPos ));

	if (index * sizeof(WCHAR) == mode->Length)
	{
	   LOG_PRINT( LOGFL_FIND_UNICODESTRING, ("return pos =%d\n",startPos + pos ));
	   return startPos + pos;
	}

	LOG_PRINT( LOGFL_FIND_UNICODESTRING, ("Last return -1\n" ));
	return -1;
}
